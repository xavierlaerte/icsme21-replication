...
png(file = "./plots/version2/out/png/c_participants.png")

participantsSATDC <- dataSATDC$num_authors
#sizeSATDI <- dataSATDI$size + 1

values <- c(participantsSATDC)

vector = c()
cont = 0
for (i in 1:(length(values))) {
  if (i < length(dataSATDC$num_authors) + 1) {
    vector[i] <- "SATD-C"
  } 
}
...
ggplot(dfchart, aes(vector2, values)) + 
  ...
  ggtitle("Engagement") + 
  ylab("# commit authors (log scale)") +
  geom_text(data = meds, aes(y = med, label = (round(med,2))),size = 8, vjust = -0.2,fontface = "bold") + 
