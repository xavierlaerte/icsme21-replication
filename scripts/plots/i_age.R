...
dataOpen <- read.csv(file="./datasets/selected/satdi_info_age_OPEN.csv", 
                      header = TRUE, sep = ";")
dataClosed <- read.csv(file="./datasets/selected/satdi_info_age_CLOSED.csv", 
                     header = TRUE, sep = ";")

png(file = "./plots/version2/out/png/i_age.png")

open <- dataOpen$duration + 1
closed <- dataClosed$duration + 1
...
for (i in 1:(length(values))) {
  if (i < length(dataOpen$duration) + 1) {
    vector[i] <- "SATD-I"
  } else {
    vector[i] <- "SATD-I"
  }
