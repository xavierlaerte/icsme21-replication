...
png(file = "./plots/version2/out/png/i_participants.png")

open <- dataOpen$participants + 1
closed <- dataClosed$participants + 1
...
ggplot(dfchart, aes(vector2, values)) + 
  ...
  ggtitle("Engagement") + 
  ylab("# participants (log scale)") +
  geom_text(data = meds, aes(y = med, label = (round(med,2) -1)),size = 8, vjust = -0.11,fontface = "bold") + 
