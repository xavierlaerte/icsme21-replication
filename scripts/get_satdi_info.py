...
if __name__ == "__main__":
    ...
    file_in_path = Path(__file__).parent / "datasets/selected/satdi.csv"
    ...
    with open(file_in_path, "r") as read_file:
        for line in read_file.readlines():
            ...
            file_name = "datasets/selected/satdi_info.csv"
            file_out_path = Path(__file__).parent / file_name
            with open(file_out_path, "a") as the_file:
                the_file.write(line.replace("\n", "").replace('"', "").replace(",", "\c$") + "\c$" \
                    + issue['bodyText'].replace("\n", "$n$") + "\c$" \
                    + issue['title'].replace("\n", "$n$") + "\c$" \
                    + str(issue['assignees']['totalCount']) + "\c$" \
                    + str(issue['comments']['totalCount']) + "\c$" \
                    + str(issue['labels']['totalCount']) + "\c$" \
                    + str(issue['participants']['totalCount']) + "\n")
